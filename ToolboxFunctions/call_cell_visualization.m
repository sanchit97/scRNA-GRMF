function [sv ] = call_cell_visualization( dataX,  actual_labels, tech, rn )
...
if(strcmp(tech,'ST10'))
    ...
    rng(rn);
    [mapped_data, mapping] = compute_mapping(dataX, tech,2);
end

sv=call_mySilhouette(mapped_data, actual_labels,length(unique(actual_labels)) );

load('../Temp/usoskin_labelNames.mat');
load('../Temp/Zeisel_labelNames.mat');
...
xlabel(tech,'FontSize',label_fontsize)
ylabel(tech,'FontSize',label_fontsize)
